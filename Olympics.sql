...
ORDER BY 3 desc;
-- 24 year old Males
-- Most popular age group for females
SELECT Sex, Age, COUNT(*) AS count 
FROM athleteevents 
GROUP BY 1,2 
HAVING Sex = 'F' 
ORDER BY 3 desc;
-- Answer is 21 years old
...
select temptable.me, count(temptable.me) as TotalGold
from
...
-- Find % of Southeast Asian countries winning a medal based on history
-- 1) Find total medal count
SELECT COUNT(*) as TotalCount 
FROM athleteevents 
WHERE Medal IN ('Gold','Silver','Bronze');
-- Total medal count: 39,783
-- 2) Use total medal count to find %
SELECT ROUND((CAST(COUNT(*) AS FLOAT)/39783)*100,2) AS SEAsian_Medal_percentage 
FROM athleteevents
WHERE Team IN ('Brunei', 'Cambodia', 'Indonesia', 'Laos', 'Malaysia', 'Mongolia', 'Philippines', 'Singapore', 'Thailand', 'Vietnam') 
AND Medal IN ('Gold', 'Silver', 'Bronze');
-- Answer is 0.29

-- Find % of Southeast Asian Countries Winning Gold based on history
-- 1) Find total gold medal count
select count(*) as TotalGoldCount
from athleteevents
where medal in ('gold');
-- Total gold count is 13,372
-- 2) Use total gold count to find %
SELECT ROUND((CAST(COUNT(*) AS FLOAT)/13372)*100,2) AS SEAsian_Gold_per 
FROM athleteevents
WHERE Team IN ('Brunei', 'Cambodia', 'Indonesia', 'Laos', 'Malaysia', 'Mongolia', 'Philippines', 'Singapore', 'Thailand', 'Vietnam') 
AND Medal IN ('Gold');
-- Answer is 0.14

-- Find % of USA winning gold to compare to Southeast Asian countries
SELECT ROUND((CAST(COUNT(*) AS FLOAT)/13372)*100,2) AS USA_Gold_per 
FROM athleteevents WHERE Team IN ('United States') AND Medal IN ('Gold');
-- USA (18.5%) is significantly greater than a Southeast Asian country winning any medal
